...
export interface AuthResponse {
  user: AuthUser;
  accessToken: string;
  /** Absent on web's response shape - added server-side for mobile clients only (see backend/src/controllers/authController.ts). */
  refreshToken?: string;
}
...
export const authApi = {
  ...
  changePassword: (input: { currentPassword: string; newPassword: string }) =>
    apiClient
      .post<{ data: { accessToken: string; refreshToken?: string } }>("/auth/change-password", input)
      .then((r) => r.data.data),
