...
export function useDeleteHabit() {
  ...
  return useMutation({
    ...
    onError: (err) => toast.error(extractErrorMessage(err, "Could not delete habit")),
  });
}
 ...
 * instant - resolveDayStatus/computeHabitStreak on the backend remain the
 * one source of truth, always overwritten by the real response in
 * onSuccess below. Mirrors frontend/src/features/habits/hooks.ts exactly.
 */
